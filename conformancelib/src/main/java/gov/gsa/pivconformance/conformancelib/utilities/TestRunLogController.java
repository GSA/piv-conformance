...
public class TestRunLogController {
        ...
        if (m_appenders == null) {
            ...
            while (i.hasNext()) {
                ...
                if (appender != null) {
                    ...
                    // For the CONFORMANCE CSV log, initialize the output file writing the header
                    // row
                    if (appender.getName().equals("CONFORMANCELOG")) {
                        ...
                        try {
                            p = new PrintStream(f);
                            p.println("Date,Test Id,Description,Expected Value,Actual Value,Test Result");
                            p.close();
    ...
    private String makeTimeStampedLogPath(TimeStampedFileAppender<ILoggingEvent> appender, Date stopTime) {
        ...
        startTs = String.format("%s_%04d%02d%02d_%02d%02d%02d", identifier, startCal.get(Calendar.YEAR),
        ...
        stopTs = String.format("%04d%02d%02d_%02d%02d%02d", endCal.get(Calendar.YEAR), endCal.get(Calendar.MONTH) + 1,
                endCal.get(Calendar.DAY_OF_MONTH), endCal.get(Calendar.HOUR_OF_DAY), endCal.get(Calendar.MINUTE),
        ...
        if (rollFile(currentLogPath, timeStampedLogPath)) {
            ...
            if (appender.getName().equals("CONFORMANCELOG")) {
                ...
                try {
                    ...
                    s_logger.debug("Couldn't write last log name to .lastlog-{}: {}", appender.getName().toLowerCase(),
                            e.getMessage());
                }
            }
        } else {
