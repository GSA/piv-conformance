...
public class PKIX_X509DataObjectTests {
        ...
        if (aiaex != null) {
            try {
                AuthorityInformationAccess aia = AuthorityInformationAccess
                        .getInstance(JcaX509ExtensionUtils.parseExtensionValue(aiaex));
                if (aiaContainsAccessMethod(aia, X509ObjectIdentifiers.ocspAccessMethod)) {
    ...
    // Sign arbitrary data using the specified key container and confirm that the
    // certificate can validate it
    ...
    void PKIX_Test_11(String oid, TestReporter reporter) {
        ...
        // always fail until 107 is addressed.");
        // fail(e);
    ...
    // Confirm that the certificate subjectAltName includes FASC-N and that it
    // matches CHUID
    ...
    // Confirm Key Management certificates for elliptic curve keys have keyAgreement
    // bit set
    // If the public key algorithm is RSA, then the keyUsage extension shall only
    // assert the
    // keyEncipherment bit. If the algorithm is Elliptic Curve key, then the
    // keyUsage extension
    ...
    // Confirm that id- fpki-common-cardAuth 2.16.840.1.101.3.2.1.3.17 OID is
    // asserted in certificate policies
            ...
            for (AccessDescription ad : descriptions) {
                if (ad.getAccessMethod().equals(X509ObjectIdentifiers.id_ad_caIssuers)) {
                    ...
                    if (location.getTagNo() == GeneralName.uniformResourceIdentifier) {
                        ...
                        // Some SSPs will also include other URLs (e.g. LDAP) that should not cause this
                        // test to fail.
                        if (!url.startsWith("http"))
                            continue;
    ...
    // specified by [RFC4122, Section 3]
    ...
    private static Map<String, X509Certificate> getCertificatesForOids(List<String> oids) {
        ...
        for (String oid : oids) {
            ...
            if (result != MiddlewareStatus.PIV_OK) {
                // this is only a warning here because it is up to the consumer of this function
                // to decide
    ...
    // The accessLocation for the id-ad-caIssuers AccessMethod is of type
    // uniformResourceIdentifier.
    ...
    // longer operative in the conformance tool
    ...
    private static Stream<Arguments> pKIX_x509TestProvider() {
        ...
        return Stream.of(Arguments.of(cert1), Arguments.of(cert2), Arguments.of(cert3), Arguments.of(cert4),
                Arguments.of(cert5));
    ...
    private static Stream<Arguments> pKIX_x509TestProvider2() {
        ...
        return Stream.of(Arguments.of(cert1, "1.3.6.1.1.16.4"), Arguments.of(cert2, "1.3.6.1.1.16.4"),
                ...
                Arguments.of(cert5, "1.3.6.1.1.16.4"));
    ...
    private static Stream<Arguments> pKIX_PIVAuthx509TestProvider2() {
        ...
        return Stream.of(
                Arguments.of(APDUConstants.X509_CERTIFICATE_FOR_PIV_AUTHENTICATION_OID, "2.16.840.1.101.3.2.1.48.11"));
    ...
    private static Stream<Arguments> pKIX_CardAuthx509TestProvider2() {
        ...
        return Stream.of(
                Arguments.of(APDUConstants.X509_CERTIFICATE_FOR_CARD_AUTHENTICATION_OID, "2.16.840.1.101.3.2.1.48.13"));
