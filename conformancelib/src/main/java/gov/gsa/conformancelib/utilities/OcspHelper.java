...
{
    private static final Logger s_logger = LoggerFactory.getLogger(OcspHelper.class);
                ...
                if (revokedStatus.getRevocationTime().compareTo(signDate) <= 0)
                {
                    throw new RevokedCertificateException(
                        "OCSP: Certificate is revoked since " +
                                revokedStatus.getRevocationTime().toString());
                }
                s_logger.info("The certificate was revoked after signing by OCSP " + ocspUrl + 
    ...
    private byte[] getKeyHashFromCertHolder(X509CertificateHolder certHolder) throws IOException
    {
        ...
        {
            // should not happen
            s_logger.error("SHA-1 Algorithm not found", ex);
            return new byte[0];
        }
    }
    ...
    public void verifyRespStatus(OCSPResp resp) throws OCSPException
    {
        ...
        if (resp != null)
        {
            ...
            case OCSPResponseStatus.INTERNAL_ERROR:
                ...
                s_logger.error("An internal error occurred in the OCSP Server!");
                break;
            case OCSPResponseStatus.MALFORMED_REQUEST:
                ...
                s_logger.error("Your request did not fit the RFC 2560 syntax!");
                break;
            case OCSPResponseStatus.SIG_REQUIRED:
                statusInfo = "SIG_REQUIRED";
                s_logger.error("Your request was not signed!");
                break;
            case OCSPResponseStatus.TRY_LATER:
                statusInfo = "TRY_LATER";
                s_logger.error("The server was too busy to answer you!");
                break;
            case OCSPResponseStatus.UNAUTHORIZED:
                statusInfo = "UNAUTHORIZED";
                s_logger.error("The server could not authenticate you!");
                break;
            case OCSPResponseStatus.SUCCESSFUL:
                break;
            default:
                statusInfo = "UNKNOWN";
                s_logger.error("Unknown OCSPResponse status code! " + status);
            }
        }
