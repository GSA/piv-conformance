...
public class PCSCWrapper implements ITransmitCounter {
	...
	public ResponseAPDU transmit(CardChannel channel, CommandAPDU cmd) throws CardException {
		...
		m_transmitCount++;
		/*
		 * ResponseAPDU rsp = null; s_apduLogger.info("Sending Command APDU: {}",
		 * Hex.encodeHexString(cmd.getBytes()).replaceAll("..(?=.)", "$0 ")); try { rsp
		 * = channel.transmit(cmd); s_apduLogger.debug("Received Response APDU: {}",
		 * Hex.encodeHexString(rsp.getBytes()).replaceAll("..(?=.)", "$0 ")); } catch
		 * (CardException e) {
		 * s_logger.error("Caught CardException {} transmitting APDU.", e.getMessage(),
		 * e); throw e; } return rsp;
		 */
